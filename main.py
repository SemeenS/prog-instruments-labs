...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('pacman_game.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger('PacmanGame')
...
class Agent:
    ...
    def __init__(self, index=0):
        self.index = index
        logger.info(f"Agent initialized with index: {self.index}")
...
class AgentState:
    ...
    def copy(self):
        logger.debug("Creating copy of AgentState")
...
class Grid:
    ...
    def copy(self):
        logger.debug("Creating copy of Grid")
    ...
    def packBits(self):
        ...
        """
        logger.debug("Packing grid to bits")
    ...
    def _unpackBits(self, bits):
        ...
        """
        logger.debug("Unpacking bits to grid data")
        cell = 0
        for packed in bits:
...
if __name__ == "__main__":
    logger.info("=== Starting Pacman Game Simulation ===")

    agent = Agent(0)

    config = Configuration((1, 1), Directions.NORTH)

    agent_state = AgentState(config, True)
    ...
    walls_grid = Grid(3, 3, False)
    walls_grid[0][0] = True  # Стена
    Actions.getPossibleActions(config, walls_grid)

    logger.info("=== Pacman Game Simulation Completed ===")